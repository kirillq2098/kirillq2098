...
Последняя существенная итерация добавила и протестировала: явный gate согласования клиента перед передачей инженеру, доменное назначение ответственных по стадиям, защиту от bootstrap race при создании выезда/задачи, синхронизацию названий SPA из авторитетного номера заказа 1С и контекст исходной заявки для дочерних ACT+DEFECT карточек.

> Последний подтверждённый CI для основного сервиса: **915 passed, 4 skipped**. LeadRouter: **590 passed**. Эти изменения подтверждены кодом и CI, но не выдаются за production deployment без отдельного runtime evidence.
